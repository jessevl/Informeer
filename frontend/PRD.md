...
> **⚠️ Historical Document** — This PRD was written when Informeer was a Miniflux frontend client. The architecture has since evolved into a fully self-contained application with its own Hono + SQLite backend. References to Miniflux below are historical context only.

> **Version:** 2.1  
> **Last Updated:** January 30, 2026  
> **Status:** In Development  
> **Type:** ~~Miniflux Client (Frontend Only)~~ Self-hosted feed reader
...
- **Magazines** — Browse and read magazine PDFs (any feed with PDF links in the configured category)
...
### 3.8 Magazines (PDF Reader)

> New feature: Browse, subscribe to, and read magazine PDFs directly in the app.

#### 3.8.1 Architecture
- **Generic PDF Detection** — Magazines are any feed in the user-configured "Magazines" category that contain PDF content. The app dynamically detects:
  - PDF enclosures (MIME type `application/pdf`)
  - PDF links in entry content (`.pdf` URLs, `/pdf/` paths)
  - Cover images from entry content (first `<img>` tag or `data-magazine-cover` attribute)
- **Miniflux Integration:**
  - Users add magazine feeds directly in Miniflux under their configured Magazines category
  - Miniflux handles polling, read/unread status, and entry management
  - Magazine issues appear as Miniflux entries with PDF enclosures or links
- **No external proxy required** — works with any RSS feed that provides PDF download links and cover images

#### 3.8.2 Magazine Discovery & Subscription
- **Add feeds** directly via the standard feed management UI
- **Any RSS feed** with PDF links works (magazine aggregator feeds, publisher feeds, etc.)
- **Filter** by subscription in the cover grid

#### 3.8.3 Magazine Cover Grid
- **Cover art gallery** — beautiful grid of magazine covers with hover effects
- **Responsive grid** — 2-6 columns depending on screen width
- **Cover details** — title, series name, publication date
- **Magazine spine effect** — subtle left-edge gradient for realism
- **Loading states** — skeleton/spinner while loading

#### 3.8.4 PDF Reader
- **Full-screen reader** with horizontal page swiping (touch) and click navigation
- **Rendering** via pdf.js (pdfjs-dist) — pages rendered to canvas
- **Navigation:**
  - Swipe left/right (mobile)
  - Click left/right 30% of canvas (desktop)
  - Arrow buttons (desktop)
  - Arrow keys (keyboard)
  - Page slider (bottom bar)
- **Zoom** controls (+/- buttons, keyboard shortcuts)
- **Fullscreen** mode (F11 / button)
- **Download** button for offline saving
- **Progress** — remembers current page per issue
- **Glassmorphism UI** — consistent with app design

#### 3.8.5 Reading Progress
- **Progress tracking** — remembers current page per issue in IndexedDB
- **Visual indicators** — progress bars and completion badges on cover grid
...
| PDF Rendering | **pdfjs-dist** (Mozilla PDF.js) | Magazine PDF viewing with canvas rendering |
