 ...
 * Fetches feed icons from the server and caches them locally
 */
...
/**
 * Convert icon data from server API to a data URL
 */
...
async function fetchIconForFeed(feedId: number, iconId?: number): Promise<string | null> {
  ...
  const fetchPromise = (async () => {
    try {
      ...
      if (iconId) {
        try {
          const icon = await api.getIcon(iconId);
      ...
      const icon = await api.getFeedIcon(feedId);
      if (icon?.data && icon?.mime_type) {
        const url = toDataUrl(icon.data, icon.mime_type);
        iconCache.icons[String(feedId)] = url;
        saveCache();
...
export const FeedIcon = memo(function FeedIcon({ feedId, iconId, size = 16, className }: FeedIconProps) {
  ...
  const wasFailed = iconCache.failed.includes(feedKey);

  // If an iconId is now provided but we previously marked this feed as failed,
  // clear the failure — the server has discovered an icon since our last attempt.
  if (wasFailed && iconId) {
    iconCache.failed = iconCache.failed.filter(id => id !== feedKey);
    saveCache();
  }
  ...
  const [error, setError] = useState(isFailed && !iconId);
  ...
  useEffect(() => {
    ...
    // Failed and no iconId to retry with
    if (isFailed && !iconId) {
      setError(true);
      return;
    }
