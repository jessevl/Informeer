...
import type { Feed, Category, ContentFetchPolicy } from '@/types/api';
...
export function EditFeedModal({ isOpen, onClose, feed, categories, onDelete }: EditFeedModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-lg bg-[var(--color-surface-base)] rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col animate-scale-in eink-shell-surface eink-modal-surface">
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="space-y-3 pt-2">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {([
                      { value: 'rss_only' as ContentFetchPolicy, label: 'RSS only', desc: 'Show content from the RSS feed as-is' },
                      { value: 'on_demand' as ContentFetchPolicy, label: 'Fetch on open', desc: 'Fetch full article when you open it' },
                      { value: 'always' as ContentFetchPolicy, label: 'Always fetch', desc: 'Fetch full article automatically during sync' },
                    ]).map((opt) => (
                      <label key={opt.value} className="flex items-start gap-3 cursor-pointer">
                        <input
                          type="radio"
                          name="contentFetchPolicy"
                          checked={contentFetchPolicy === opt.value}
                          onChange={() => setContentFetchPolicy(opt.value)}
                          className="mt-0.5 w-4 h-4 border-[var(--color-border-default)] text-[var(--color-accent-fg)] focus:ring-[var(--color-accent-fg)]"
                        />
                        <div>
          ...
          <div className="p-3 rounded-lg bg-[var(--color-surface-secondary)] text-xs text-[var(--color-text-tertiary)] space-y-1">
            <div className="flex justify-between">
              <span>Last checked</span>
              <span className="text-[var(--color-text-secondary)]">
                {feed.checked_at ? formatRelativeTime(feed.checked_at) : 'Never'}
              </span>
            </div>
            <div className="flex justify-between">
              ...
              <span className="text-[var(--color-text-secondary)]">
                {feed.next_check_at 
                  ? new Date(feed.next_check_at) <= new Date() ? 'Due now' : formatRelativeTime(feed.next_check_at)
                  : 'Pending'}
              </span>
            </div>
          </div>
