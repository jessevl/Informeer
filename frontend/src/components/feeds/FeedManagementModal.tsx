...
import type { Feed, Category } from '@/types/api';
...
export function FeedManagementModal({ isOpen, onClose, feeds, categories }: FeedManagementModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Modal */}
      <div className="relative w-full max-w-3xl bg-[var(--color-surface-base)] rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col animate-scale-in eink-shell-surface eink-modal-surface">
        {/* Header */}
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        {cat.is_system ? (
                          <span className="text-[10px] text-[var(--color-text-tertiary)] italic px-1">system</span>
                        ) : isEditing ? (
                          <>
                            <button
          ...
          {feeds.map(feed => {
            ...
            return (
              ...
              >
                ...
                {!isEditing && feed.checked_at && (
                  <span
                    title={`Last checked: ${formatRelativeTime(feed.checked_at)}${feed.next_check_at ? `\nNext check: ${new Date(feed.next_check_at) <= new Date() ? 'Due now' : formatRelativeTime(feed.next_check_at)}` : ''}`}
                    className="text-xs text-[var(--color-text-disabled)] tabular-nums"
                  >
                    {formatRelativeTime(feed.checked_at)}
                  </span>
                )}
