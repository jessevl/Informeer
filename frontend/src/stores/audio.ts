...
import type { Enclosure, Entry } from '@/types/api';
...
interface AudioState {
  ...
  // Sleep timer
  sleepTimerMinutes: number | null; // null = off, number = minutes remaining
  ...
  persist(
    (set, get) => ({
      ...
      markAsListened: (entryId) => {
        ...
        // Also mark as read in Informeer
        api.updateEntries({ entry_ids: [entryId], status: 'read' }).catch(console.error);
      },
      ...
      syncProgress: async () => {
        ...
        set({ isSyncing: true });
        try {
          await api.updateEnclosureProgress(currentEnclosure.id, Math.floor(currentTime));
        } catch (error) {
      ...
      setSleepTimer: (minutes) => {
        if (minutes === null) {
          ...
          set({ sleepTimerMinutes: minutes, sleepTimerEndTime: Date.now() + minutes * 60 * 1000 });
        }
      },
